...
function createNewRecord() {
  ...
  const currentDate = current.slice(8, 10) + '/' 
  + current.slice(5, 7) + '/' 
  + current.slice(0, 4);

  const currentTime = cTime.getHours() + ':' + cTime.getMinutes();
  ...
  if (validate(currentName, currentAge)) {
    ...
    showSnakeBar("Item added Successfully!", "success", 1500);
    loadDataFromLocalStorage();
    emptyFieldBox();
  } else {
    showSnakeBar("Error adding the Item", "error", 1500);
    emptyFieldBox();
  }
}
...
function deleteDataFromLocalStorage(records_id) {
  $("#deleteModelBtn").on("click", function () {
    ...
    localStorage.setItem("crudtable", JSON.stringify(tableData));
    showSnakeBar("Item deleted Successfully!", "success", 1500);
    loadDataFromLocalStorage();
  });
  $("#deleteCnlModelBtn").on("click", function () {
    showSnakeBar("Cancelled deleting the Item", "cancelled", 1500);
    loadDataFromLocalStorage();
  });
}
...
function updateButtonPress() {
  ...
  emptyFieldBox();
  showSnakeBar("Successfully updated the Item", "success", 1500);
}

function updateCancelButton() {
  displayNoneEditMarkup();
  emptyFieldBox();
  showSnakeBar("Cancelled updating the Item", "cancelled", 1500);
}
